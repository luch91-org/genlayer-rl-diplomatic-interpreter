...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the DiplomaticInterpreter RL agent.")
    ...
    # Gamma is deliberately moderate: each draft's acceptance reward arrives
    # immediately, so the payoff is fairly myopic. A high gamma (>=0.9) lets
    # optimistically-initialized Q-values chain across the polarization
    # transitions and converge on a low-immediate-reward action that merely
    # *looks* like it leads somewhere good -- picking the 3/10 affirmation
    # while the room is still hot. Discounting the future to 0.6 keeps the
    # best-immediate-acceptance-per-bucket policy (the real tact lesson) on
    # top; anything in ~0.3-0.8 converges identically (see docs/tutorial.md).
    parser.add_argument("--gamma", type=float, default=0.6)
    parser.add_argument("--epsilon-decay", type=float, default=0.99)
    parser.add_argument("--epsilon-min", type=float, default=0.01)
    # Optimistic Q-initialization above the realistic per-episode return
    # forces the agent to try every compromise in every polarization bucket;
    # since the best compromise is state-dependent, this matters for finding
    # the tact-aware policy rather than a single-archetype habit.
    parser.add_argument("--optimistic-init", type=float, default=50.0)
    parser.add_argument("--seed", type=int, default=None)
    # Start each mock episode from a random polarization so every bucket is an
    # on-policy start state. Without it, the fixed-0.80 start only ever walks
    # one trajectory (high -> low -> resolved), leaving "medium" untrained --
    # and the live chain WILL land there when the LLM scores a strong
    # compromise 8.0 (polarization 0.50) instead of the mock's 8.8 (0.46).
    parser.add_argument(
        "--random-start",
        dest="random_start",
        action="store_true",
        default=True,
        help="Mock only: seed each episode from a random polarization (default on).",
    )
...
def make_env(args: argparse.Namespace):
    if args.env == "mock":
        return MockEnv(max_steps=args.max_steps, seed=args.seed, random_start=args.random_start)
    if not args.address:
...
def run_episode(env, agent: QLearningAgent) -> tuple[float, str]:
    ...
    while not env.is_episode_done():
        ...
        # strip internal keys (e.g. _template_id) so the contract only ever
        # sees its documented action schema
